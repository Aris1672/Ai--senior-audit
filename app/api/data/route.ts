...
case "update_session_paid": {
  ...
  return NextResponse.json({ success: true });
      ...
      case "generate_report": {
        ...
        const criticalCt   = allFindings.filter((f: any) => f.risk_level === "КРИТИЧНО").length;
        const majorCt      = allFindings.filter((f: any) => f.risk_level === "СУЩЕСТВЕННО").length;
        const minorCt      = allFindings.filter((f: any) => f.risk_level === "НЕСУЩЕСТВЕННО").length;

        const auditDate  = new Date(sessionRaw.created_at).toLocaleDateString("ru-RU", { day: "numeric", month: "long", year: "numeric" });
        ...
        function findingsSection(level: string): any[] {
          const items = allFindings.filter((f: any) => f.risk_level === level);
        ...
        const docDefinition: any = {
          ...
          content: [
            ...
            { text: companyName, fontSize: 20, color: colors.blue, margin: [0, 0, 0, 8] },
            ...
            { text: "Сводная информация", fontSize: 14, bold: true, color: colors.white, margin: [0, 0, 0, 12] },
            {
              table: {
                ...
                body: [
                  ...
                  [{ text: "Транзакций проверено", color: colors.light, margin: [8,6,8,6] }, { text: sessionRaw.transactions_ct?.toString() || "—", color: colors.blue, bold: true, margin: [8,6,8,6] }],
                  ...
                  [{ text: "Стоимость аудита",      color: colors.light, margin: [8,6,8,6] }, { text: sessionRaw.cost_rub ? sessionRaw.cost_rub.toLocaleString("ru-RU", { style: "currency", currency: "RUB", maximumFractionDigits: 0 }) : "—", color: colors.amber, bold: true, margin: [8,6,8,6] }],
                ],
              },
